...
nullptr; } MWArmneonTarget::MWCNNLayerImpl* owningLayerImpl = 
static_cast<MWArmneonTarget::MWCNNLayerImpl*>(owningLayer->getImpl()); int 
layerOpPortIndex = aTensor->getSourcePortIndex(); if (owningLayerImpl == 
nullptr) { if (owningLayer->isCustomLayer()) { return nullptr; } else { int 
inIndex = owningLayer->getInPlaceIndex(layerOpPortIndex); bool isInplaceLayer = 
...
(owningLayer == nullptr) { return nullptr; } MWArmneonTarget::MWCNNLayerImpl* 
owningLayerImpl = 
static_cast<MWArmneonTarget::MWCNNLayerImpl*>(owningLayer->getImpl()); int 
layerOpPortIndex = aTensor->getSourcePortIndex(); if (owningLayerImpl == 
nullptr) { if (owningLayer->isCustomLayer()) { return nullptr; } else { int 
...
else { assert(isInplaceLayer); return MWACLUtils::getLayerOpArmTensorsharedPtr( 
owningLayer->getInputTensor(inIndex)); } } } else { auto 
layerOpArmTensorSharePtr = 
